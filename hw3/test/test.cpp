...
BOOST_AUTO_TEST_CASE(test_allocator_basic) {
  reset_counters();
  {
    auto c = TheContainer<int, logging_allocator<TheContainerItem<int>>>{};
    c.push_back(1);
  }
  check_counters(1);
}
...
BOOST_AUTO_TEST_CASE(test_allocator_copy) {
  reset_counters();
  {
    auto c1 = TheContainer<int, logging_allocator<TheContainerItem<int>>>{};
    c1.push_back(1);
    c1.push_back(2);
    auto c2 = TheContainer<int, logging_allocator<TheContainerItem<int>>>{c1};
  }
  check_counters(4);
}
...
BOOST_AUTO_TEST_CASE(test_allocator_move) {
  reset_counters();
  {
    ...
    c1.push_back(2);
    c1.push_back(3);
    auto c2 = TheContainer<int, logging_allocator<TheContainerItem<int>>>{std::move(c1)};
    auto it = c2.begin();
    BOOST_CHECK(*it == 1);
    BOOST_CHECK(*(++it) == 2);
    BOOST_CHECK(*(++it) == 3);
